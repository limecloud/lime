...
### 2B.2 首批内置 Builder Skill 已迁入

persona Builder Skills 已迁入内置目录；P1 是个人 IP，P2 是品牌人设：
...
brand-persona-knowledge-builder/
  SKILL.md                                 # 品牌人设整理工作流
  references/
    brand-persona-template.md              # 品牌人设章节骨架
    interview-questions.md                 # 品牌定位 / 语气 / 边界访谈问题
    quality-checklist.md                   # 品牌 tone / 危机 / 禁忌自检
  agents/
    openai.yaml                            # 模型适配
```

data Builder Skills 也已进入同一内置目录，其中既包括运营 playbook，也包括产品事实、组织经验和增长策略：
...
**v2 PRD 里 §5、§6.1 的内容已经迁入各 Builder Skill 的资源文件，而不是继续写在 PRD 里**。PRD 只负责声明"哪个 Skill 负责生成哪类 pack"。
...
| Skill 名称                                    | 生成的 pack 类型          | family  | 状态               |
| --------------------------------------------- | ------------------------- | ------- | ------------------ |
| `personal-ip-knowledge-builder`               | personal-profile          | persona | **current / 内置** |
| `content-operations-knowledge-builder`        | content-operations        | data    | **current / 内置** |
| `private-domain-operations-knowledge-builder` | private-domain-operations | data    | **current / 内置** |
| `live-commerce-operations-knowledge-builder`  | live-commerce-operations  | data    | **current / 内置** |
| `campaign-operations-knowledge-builder`       | campaign-operations       | data    | **current / 内置** |
| `brand-persona-knowledge-builder`             | brand-persona             | persona | **current / 内置** |
| `brand-product-knowledge-builder`             | brand-product             | data    | **current / 内置** |
| `organization-knowhow-knowledge-builder`      | organization-knowhow      | data    | **current / 内置** |
| `growth-strategy-knowledge-builder`           | growth-strategy           | data    | **current / 内置** |

运营类知识库不新增 family，只作为 data 族的运营 playbook 型 pack；当前已内置内容运营、私域 / 社群运营、直播运营和活动运营四类 Builder Skill。品牌人设是 persona 族，品牌产品、组织经验和增长策略是 data 族，只是沉淀的事实对象不同。
...
2. Builder Skill **不复制进 pack**。它作为 Agent Skill Bundle 存在于 `src-tauri/resources/default-skills/`、workspace-local skills 或远端 `SkillCatalog`；pack 只在 `KNOWLEDGE.md.metadata` 与 `runs/compile-*.json` 里记录使用过的 skill provenance。
...
- 已完成 `personal-ip-knowledge-builder` 内置迁移、`SkillCatalog.entries(kind=skill)` 投影和 `knowledge_compile_pack` 的命令层 Runtime Binding seam。
- 已内置 `brand-persona-knowledge-builder`，补齐 persona 族第二个 Builder Skill；品牌人设 pack 会写入 `agent-skill / brand-persona-knowledge-builder` provenance，并按 `runtime.mode=persona` 进入 Resolver。
- 已内置七类 data Builder Skills：`brand-product-knowledge-builder`、`organization-knowhow-knowledge-builder`、`growth-strategy-knowledge-builder`、`content-operations-knowledge-builder`、`private-domain-operations-knowledge-builder`、`live-commerce-operations-knowledge-builder`、`campaign-operations-knowledge-builder`，并接入默认 Skill 安装链与 seeded SkillCatalog 投影。
- 个人 IP pack 已写入 `metadata.producedBy.kind=agent-skill`、`name=personal-ip-knowledge-builder`、Skill digest 与 `bundlePath`；命令层会按 pack metadata 生成 `KnowledgeBuilderRuntimePlan`，调用 `execute_named_skill`，成功时用 Builder Skill 输出写回 `documents/<doc>.md`。
- data pack 现在同样按 pack type 写入对应 `agent-skill` provenance；Runtime Binding 成功时使用 Skill 输出，失败时记录真实 `attempted/status/error` 并回退 deterministic adapter。
- `runtimeBinding.executed` 只在模型级 Skill 成功返回 `KnowledgeBuilderSkillOutput` 时为 `true`；Provider / AppHandle / 执行失败时真实记录 `attempted/status/error`，并回退确定性 adapter，不伪造执行成功。
- 前端回归、DevBridge mock、后端 `compiledView` 与 `smoke:knowledge-gui` 已退出 `compiled/brief.md` current 路径：页面 mock、Resolver mock、浏览器 fallback mock、compile response / pack detail API 与 File Manager smoke 现在改读 document-first splits 或真实 source 文件；新 pack 不再写入 `compiled/brief.md`，该文件只作为旧 pack fallback 保留。
...
| --------------------------- | --------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------- |
| `personal-profile`          | `personal-ip`               | `personal-ip-knowledge-builder`               | `src-tauri/resources/default-skills/personal-ip-knowledge-builder/`               |
| `brand-persona`             | `brand-persona`             | `brand-persona-knowledge-builder`             | `src-tauri/resources/default-skills/brand-persona-knowledge-builder/`             |
| `brand-product`             | `brand-product`             | `brand-product-knowledge-builder`             | `src-tauri/resources/default-skills/brand-product-knowledge-builder/`             |
| `organization-knowhow`      | `organization-knowhow`      | `organization-knowhow-knowledge-builder`      | `src-tauri/resources/default-skills/organization-knowhow-knowledge-builder/`      |
| `content-operations`        | `content-operations`        | `content-operations-knowledge-builder`        | `src-tauri/resources/default-skills/content-operations-knowledge-builder/`        |
| `private-domain-operations` | `private-domain-operations` | `private-domain-operations-knowledge-builder` | `src-tauri/resources/default-skills/private-domain-operations-knowledge-builder/` |
| `live-commerce-operations`  | `live-commerce-operations`  | `live-commerce-operations-knowledge-builder`  | `src-tauri/resources/default-skills/live-commerce-operations-knowledge-builder/`  |
| `campaign-operations`       | `campaign-operations`       | `campaign-operations-knowledge-builder`       | `src-tauri/resources/default-skills/campaign-operations-knowledge-builder/`       |
| `growth-strategy`           | `growth-strategy`           | `growth-strategy-knowledge-builder`           | `src-tauri/resources/default-skills/growth-strategy-knowledge-builder/`           |
...
- 创建资料时的"模板选择"（个人 IP / 品牌人设 / 品牌产品 / 组织 Know-how / 内容运营 / 私域与社群运营 / 直播运营 / 活动 Campaign / 增长策略）
- 整理失败时的"用其他模板重试"
- 高级用户视图才显示"使用了 `<builder-skill-name>` v1.0.0 整理"
...
| P2    | 品牌人设             | `brand-persona`             | persona  | `brand-persona-knowledge-builder`             | current，已内置 |
| P3    | 品牌产品             | `brand-product`             | data     | `brand-product-knowledge-builder`             | current，已内置 |
| P3    | 组织 Know-how        | `organization-knowhow`      | data     | `organization-knowhow-knowledge-builder`      | current，已内置 |
| P3    | 内容运营             | `content-operations`        | data     | `content-operations-knowledge-builder`        | current，已内置 |
| P3    | 私域 / 社群运营      | `private-domain-operations` | data     | `private-domain-operations-knowledge-builder` | current，已内置 |
| P4    | 直播运营             | `live-commerce-operations`  | data     | `live-commerce-operations-knowledge-builder`  | current，已内置 |
| P4    | 活动 / Campaign 运营 | `campaign-operations`       | data     | `campaign-operations-knowledge-builder`       | current，已内置 |
| P4    | 增长策略             | `growth-strategy`           | data     | `growth-strategy-knowledge-builder`           | current，已内置 |
...
事实源：`src-tauri/resources/default-skills/personal-ip-knowledge-builder/`
...
### 6.3 `brand-persona-knowledge-builder`（P2 current）

事实源：`src-tauri/resources/default-skills/brand-persona-knowledge-builder/`

```text
brand-persona-knowledge-builder/
  SKILL.md
  references/
    brand-persona-template.md
    interview-questions.md
    quality-checklist.md
  agents/
    openai.yaml
...
该 Skill 已自带品牌人设整理工作流、品牌人设章节骨架、品牌 tone / 禁忌 / 危机回应自检，以及品牌定位、受众、表达边界补充问题。

**可支持任务**：品牌故事改写、公众号 / 视频号内容、小红书 / 抖音短内容、客服话术、危机回应草稿。

### 6.4 `brand-product-knowledge-builder`（P3 current）

事实源：`src-tauri/resources/default-skills/brand-product-knowledge-builder/`

用户场景：把产品事实、参数、价格、渠道、合规边界和客服 FAQ 整理成 data 族 pack。

该 Skill 已内置：

- `SKILL.md`：产品资料整理工作流与 Runtime Binding 契约。
- `agents/openai.yaml`：catalog / UI 展示信息。
- `references/brand-product-template.md`：产品事实章节骨架。
- `references/brand-product-quality-checklist.md`：规格、价格、权益、卖点证据和合规边界自检。
...
### 6.5 `organization-knowhow-knowledge-builder`（P3 current）

事实源：`src-tauri/resources/default-skills/organization-knowhow-knowledge-builder/`

用户场景：把组织 SOP、客户分层、成功/失败案例、异常升级路径、术语和不可回答边界整理成 data 族 pack。

该 Skill 已内置：

- `SKILL.md`：组织经验整理工作流与 Runtime Binding 契约。
- `agents/openai.yaml`：catalog / UI 展示信息。
- `references/organization-knowhow-template.md`：组织经验章节骨架。
- `references/organization-knowhow-quality-checklist.md`：角色职责、SOP、检查点、升级机制和复盘改进自检。

**可支持任务**（Phase 3）：新员工培训材料、客户问答、方案生成、质检复盘、SOP 自动化执行。

### 6.6 `growth-strategy-knowledge-builder`（P4 current）

事实源：`src-tauri/resources/default-skills/growth-strategy-knowledge-builder/`

用户场景：把业务背景、增长假设、指标体系、用户分层、渠道矩阵、内容策略、活动节奏和复盘记录整理成增长策略 pack。

该 Skill 已内置：

- `SKILL.md`：增长策略整理工作流与 Runtime Binding 契约。
- `agents/openai.yaml`：catalog / UI 展示信息。
- `references/growth-strategy-template.md`：增长策略章节骨架。
- `references/growth-strategy-quality-checklist.md`：目标、指标、渠道、实验、资源和停止条件自检。

**可支持任务**（Phase 4）：10 倍增长策略、30/60/90 天计划、渠道投放节奏、销售转化 SOP、老板汇报版方案。

### 6.7 运营类 Builder Skills（current，data family）

运营类知识库不是“个人 IP”的变体，也不是单纯产品资料；它们沉淀的是**可重复执行的运营 playbook**。首批按业务动作拆成 4 个 Skill，避免一个“万能运营模板”过大、不可验证。

事实源已进入 `src-tauri/resources/default-skills/*-operations-knowledge-builder/`；每个 Skill 包至少包含 `SKILL.md`、`agents/openai.yaml`、`references/<template>.md` 与 `references/<quality-checklist>.md`。
...
1. Builder Skill 存放在 `src-tauri/resources/default-skills/`、workspace-local skills 或远端 `SkillCatalog`；不存放在 `src-tauri/resources/default-templates/knowledge/`。
  ...
  "splits": [
    {
      ...
      "title": "第一章 人物档案与基本信息",
      "order": 1,
      "path": "compiled/splits/谢晶_个人IP知识库v1.0/001_第一章_人物档案与基本信息.md",
      "tokens": 2400,
      "sourceAnchors": ["sources/访谈稿_20260201.transcript.md#L1-L120"]
    },
    {
      "id": "appendix02",
      "title": "附录二 智能体应用指南",
      "order": 999,
      "path": "compiled/splits/谢晶_个人IP知识库v1.0/附录二_智能体应用指南.md",
      "tokens": 600,
      "sourceAnchors": []
    }
  ],
...
1. 将 `src-tauri/resources/default-skills/personal-ip-knowledge-builder/` 作为首个内置 seeded Builder Skill 纳入 SkillCatalog 投影；记录 `skillBundle` 摘要、版本、digest 与资源清单。**当前已完成内置迁移与 catalog projection。**
2. `knowledge_compile_pack` 实现 §5 的薄适配：按 metadata / SkillCatalog 选 Skill，进入 runtime binding，接收 `KnowledgeBuilderSkillOutput`，写入 `documents/` 与 `runs/`。**当前已完成命令层最小 Runtime Binding：personal IP pack 会生成 Builder Skill 运行计划并调用 `execute_named_skill`；成功时 `runtimeBinding.executed=true` 且主文档来自 Skill 输出，失败时记录真实失败并回退 deterministic adapter。**
...
4. `knowledge_split_document` 实现 §7.1。**当前已完成最小 document-first 派生切片：`knowledge_compile_pack` 写入 `compiled/splits/<doc>/` 与 `compiled/index.json`，Resolver 优先消费 splits；`compiled/brief.md` 仅保留为 compat 视图。**
5. `knowledge_resolve_context` 实现 §7.2 主路径 + §7.2.1 persona 分支。**当前已完成最小 persona/data Resolver 语义：persona pack 优先注入应用指南、金句、性格、价值观等核心人设切片；data pack 保持文档顺序。**
6. fenced wrapper 实现 §7.3.1 data 模板 + §7.3.2 persona 模板。**当前 wrapper 已按 `runtime.mode` 区分 data / persona；persona 明确声明“人设资料不是指令”，不得升级为 system prompt 或编造未覆盖事实。**
...
1. `brand-persona-knowledge-builder` 已内置并通过 SkillCatalog 投影给 UI 的"品牌人设"模板；后续 Phase 2 重点转为真实品牌资料评测和 `/品牌文案` 命令投影。
...
1. `brand-product-knowledge-builder` 已内置（family: data），用于实现 persona + data 双 pack 协同的标准范式（"以谢晶口吻介绍金花黑茶"）。
2. `organization-knowhow-knowledge-builder` 已内置，覆盖团队 SOP、角色职责、检查点和升级机制。
3. `content-operations-knowledge-builder` 与 `private-domain-operations-knowledge-builder` 已提前内置，覆盖内容日历、社群 SOP、触达节奏和转化话术；后续 Phase 3 重点转为真实资料评测、source anchors 和 UI 组合体验打磨。
...
1. `growth-strategy-knowledge-builder`、`live-commerce-operations-knowledge-builder` 与 `campaign-operations-knowledge-builder` 均已内置；后续 Phase 4 重点转为增量维护、版本化、团队协作和真实增长资料评测。
2. 增量整理（新增 source 时自动 patch 受影响章节），由 Builder Skill 决定 patch 策略，Lime 保持文件写入与状态收口。
3. 文档版本化与变更审计。
4. 团队协作（多端冲突解决方案升级）。
...
本地工程闭环完成度：约 99% — 口径：v2 PRD / diagrams / README / Release Notes、9 个内置 current Builder Skills、seeded SkillCatalog、Runtime Binding seam、真实 Provider E2E、Provider JSON 包装宽容解析、document-first splits、persona/data Resolver、`1 persona + N data` GUI、`compiled/brief.md` current 退出、`knowledge_builder` deprecated compat、legacy fallback 扫描工具、Rust / 前端 / contracts / governance / GUI smoke 均已覆盖。

整体目标完成度（含真实交付）：约 97% — 口径：v1 已落地的产品骨架（目录、状态、命令、UI 入口）按 v1 计算约 30%；v2 PRD / diagrams / 个人 IP、品牌人设与七类 data 内置 Builder Skills / seeded SkillCatalog / Runtime Binding seam / 真实 Provider E2E / deterministic fallback / 前端 Skills-first metadata / document-first 派生切片 / persona-data Resolver / 多 pack Resolver / data 自动携带 persona metadata / 输入框显式追加 N 个 data 协同资料 / KnowledgePage 完整多选 chooser 与 smoke 覆盖 / 前端、DevBridge mock、后端 `compiledView` 与新 pack 写入退出 `compiled/brief.md` current 路径 / `knowledge_builder` deprecated Skill 指令层委托 / 标准 pack 类型禁止回流 compat builder / 个人 IP 与八类非个人 pack 真实风格 fixture 回归 / 旧 pack brief fallback 诊断与重新整理迁移回归合计约 67%；剩余约 3% 是真实长资料人工质量评测、真实用户 workspace 历史 fallback 清零后删除 `knowledge_builder`，以及需用户确认的 GitHub commit / tag / push / release。

### 17.1 完成度审计清单（prompt-to-artifact）

这张表是 Knowledge v2 是否完成的可审计事实源。任何后续声称“完成”都必须逐项回到这里核对，而不是只看某个测试是否为 green。

| 明确要求 / 交付物 | 版本化证据 | 验证证据 | 结论 |
| --- | --- | --- | --- |
| PRD v2 是 current，且采用 Skills-first | `docs/roadmap/knowledge/prd-v2.md`、`docs/roadmap/knowledge/prd.md` banner | `rg -n "Skills-first|Agent Skills|Agent Knowledge" docs/roadmap/knowledge README.md RELEASE_NOTES.md` | 已完成 |
| 可视化文档同步架构图、流程图、时序图、UI 原型和用户故事 | `docs/roadmap/knowledge/prd-v2-diagrams.md` | `rg -n "Skills-first|document-first|runtime.mode|metadata.producedBy|1 persona" docs/roadmap/knowledge/prd-v2-diagrams.md` | 已完成 |
| 参考 Agent Skills Apache 核心标准，不发明 Lime 私有整理引擎 | `src-tauri/resources/default-skills/*-knowledge-builder/SKILL.md` frontmatter 写 `license: Apache-2.0`；`§2B` 说明参考 `agentskills.io` | Builder Skill frontmatter 审计脚本输出 `current_builder_count=9` 且全部通过 | 已完成 |
| Agent Skills 负责“怎么生产和维护知识” | 9 个 Builder Skill 的 `SKILL.md`、`references/`、`agents/openai.yaml` | `find src-tauri/resources/default-skills/*knowledge-builder -maxdepth 2 -type f`；默认 Skill Rust 测试 | 已完成 |
| Agent Knowledge 负责“知识产物长什么样、如何安全进入上下文” | `KNOWLEDGE.md` profile、runtime、metadata 设计写入 `§3`、`§7`、`§11` | `lime-knowledge` crate 18 个单测全部通过 | 已完成 |
| 不只个人 IP，还包含运营类知识库 | `content-operations`、`private-domain-operations`、`live-commerce-operations`、`campaign-operations` 四类 Builder Skills | `builtin_data_builder_specs_should_cover_operations_product_org_and_growth` | 已完成 |
| persona 族覆盖个人 IP 与品牌人设 | `personal-ip-knowledge-builder`、`brand-persona-knowledge-builder` | `brand_persona_compile_should_use_embedded_builder_skill_provenance`、默认 Skill 同步测试 | 已完成 |
| data 族覆盖品牌产品、组织经验、增长策略和运营类 | 七类 data Builder Skills | `operations_compile_should_use_embedded_builder_skill_provenance`、`non_personal_builder_skills_should_compile_realistic_sources_to_document_first_packs` | 已完成 |
| Builder Skills 迁入内置默认 Skills | `src-tauri/resources/default-skills/`、`src-tauri/src/skills/default_skills.rs`、`src/lib/base-setup/seededServiceSkillPackage.ts` | `cargo test -p lime --lib default_skills --no-default-features`、`seededServiceSkillPackage.test.ts` | 已完成 |
| SkillCatalog 有 seeded projection | `src/lib/base-setup/seededServiceSkillPackage.ts`、`src/lib/api/skillCatalog.ts` | `npm test -- src/lib/base-setup/seededServiceSkillPackage.test.ts src/lib/api/skillCatalog.test.ts` | 已完成 |
| `knowledge_compile_pack` 是调用 Builder Skill 的薄适配 | `src-tauri/src/commands/knowledge_cmd.rs`、`src-tauri/crates/knowledge/src/lib.rs` | `personal_ip_compile_should_accept_successful_builder_runtime_output`、真实 Provider E2E run `provider-e2e-persona-fixed` | 已完成 |
| 新 pack 使用 `profile=document-first` | `src-tauri/crates/knowledge/src/lib.rs` metadata 写入逻辑 | `import_compile_and_resolve_pack_should_use_standard_structure` | 已完成 |
| 产物写入 `documents/`、`compiled/splits/`、`compiled/index.json` | `src-tauri/crates/knowledge/src/lib.rs` split 逻辑 | `import_compile_and_resolve_pack_should_use_standard_structure`、GUI smoke | 已完成 |
| 新 pack 退出 `compiled/brief.md` current 路径 | `§7`、`§17`、`scripts/knowledge-legacy-fallback-report.mjs` | `recompiling_legacy_pack_should_remove_brief_fallback`、`npm run knowledge:legacy-fallback-report -- --working-dir "." --json` | 已完成 current 退出 |
| Resolver 只消费 KnowledgePack，不执行 Builder Skill | `§7.2`、`§7.3`、`src-tauri/crates/knowledge/src/lib.rs` | `resolve_context_should_merge_persona_before_data_packs`、`npm run verify:gui-smoke -- --reuse-running` | 已完成 |
| 支持 `1 persona + N data` | `KnowledgePage` chooser、输入框资料中枢、`knowledge_pack.packs[]` metadata | `KnowledgePage.test.tsx`、`Inputbar/index.test.tsx`、`scripts/knowledge-gui-smoke.mjs` | 已完成 |
| GUI 主路径可交付 | 首页 / 输入框 / File Manager / KnowledgePage / Agent 输出沉淀 | `npm run verify:gui-smoke -- --reuse-running` 覆盖 `knowledge-gui` 与 `design-canvas` | 已完成 |
| `knowledge_builder` 只保留 compat / deprecated | `src-tauri/resources/default-skills/knowledge_builder/SKILL.md` | `standard_pack_types_should_not_fall_back_to_knowledge_builder` | 已完成，待最终删除 |
| 历史 fallback 有扫描与退出依据 | `scripts/knowledge-legacy-fallback-report.mjs`、`package.json` script、README 说明 | 当前仓库 workspace 扫描 `total=0`；真实用户 workspace 需另跑 | 本地已完成，真实迁移待清零 |
| Release Notes / README 更新 | `README.md`、`RELEASE_NOTES.md`、`docs/README.md` | `rg -n "knowledge:legacy-fallback-report|1 persona|content-operations|knowledge_builder" README.md RELEASE_NOTES.md docs/README.md` | 已完成 |
| 上游 Agent Knowledge v0.6 标准发布 | `/Users/coso/Documents/dev/ai/limecloud/agentknowledge` | `git tag --points-at HEAD` 输出 `v0.6.0`；`origin/main` 同步 | GitHub Release 已完成，npm 待认证 |
| 真实 Provider E2E | `docs/roadmap/knowledge/evidence/provider-e2e-20260508.json`（原始临时 workspace `/var/folders/87/s6cpr7hd1_v43cs833x4s_900000gn/T/lime-knowledge-provider-e2e-fixed-5Dvto6`） | DevBridge + `custom-da3283c4-8405-45e9-81cd-12991ffdf41c` / `claude-sonnet-4-6`：`runtimeBinding.executed=true`、`status=succeeded`、`documents/provider-e2e-persona-fixed.md`、`compiled/index.json`、25 个 splits、persona fenced context 无 warning | 已完成 |
| 真实 Provider E2E 可复用脚本 | `scripts/knowledge-provider-e2e.mjs`、`package.json` script `knowledge:provider-e2e` | `node --check scripts/knowledge-provider-e2e.mjs`；无 `--allow-external-provider` 时拒绝外部调用；`--help` 记录风险开关 | 已完成 |
| Knowledge-only 发布范围只读审计 | `scripts/knowledge-release-scope-report.mjs`、`package.json` script `knowledge:release-scope-report`、`docs/roadmap/knowledge/evidence/release-scope-report-20260508.json` | `node --check scripts/knowledge-release-scope-report.mjs`；报告当前 dirty scope：knowledge=75、nonKnowledge=30、mixed=0、unknown=0 | 已完成审计，仍需人工确认发布范围 |
| 完成度审计 artifact | `docs/roadmap/knowledge/completion-audit-20260508.md` | prompt-to-artifact checklist 已覆盖用户显式要求、证据、缺口和发布阻塞 | 已完成审计，目标未 100% |
| Provider JSON 包装鲁棒性 | `src-tauri/crates/knowledge/src/lib.rs` 宽容提取不严格 JSON fence 里的 `primaryDocument.content` | `builder_runtime_parser_should_extract_loose_json_fenced_content`；二次真实 E2E 文档 `startsWithMarkdownHeading=true`、`startsWithJsonFence=false` | 已完成 |
| Provider 输出质量评测（短资料） | `docs/roadmap/knowledge/evidence/provider-e2e-quality-review-20260508.md` | 对照 `personal-ip-knowledge-builder` quality checklist：事实完整性 / 防编造 / 结构 / 可调用性 / 风格 / runtime 安全均 PASS，多场景话术 PARTIAL | 已完成短资料评测 |
| 真实长资料人审 | 需要真实个人访谈、品牌、产品、组织、增长、运营资料评分 | 已有 golden sample / fixture、一次短资料真实 Provider E2E 与短资料质量评测，不等于真实长资料批量人审 | 未完成 |
| GitHub commit / tag / push / release | 需要 Git 操作和 GitHub Release 证据 | 按仓库规则属于高风险操作，尚未获得本轮明确确认 | 未完成 / 需确认 |

### 17.2 发布前范围审计（避免误带无关改动）

当前 worktree 不是干净状态；如果要做 Knowledge v2 发布，必须先确认发布范围，不能直接把所有 dirty files 一次性提交。

| 分类 | 文件范围 | 发布建议 |
| --- | --- | --- |
| Knowledge v2 主线候选 | `docs/roadmap/knowledge/*`、`README.md`、`RELEASE_NOTES.md`、`docs/README.md`、`package.json`、`scripts/knowledge-*`、`src-tauri/crates/knowledge/`、`src-tauri/resources/default-skills/*knowledge*`、`src-tauri/src/commands/knowledge_cmd.rs`、`src-tauri/src/dev_bridge/dispatcher/{knowledge,files}.rs`、`src-tauri/src/skills/default_skills.rs`、`src-tauri/crates/core/src/models/{mod,skill_model}.rs`、`src/features/knowledge/`、输入框资料中枢、File Manager 资料入口、`KnowledgePack` runtime metadata、SkillCatalog seeded projection、DevBridge mock / tauri mock | 可进入 Knowledge-only 发布候选；提交前仍需人工复核 staged / unstaged 是否混杂 |
| Knowledge v2 关联但需人工确认 | `src-tauri/src/commands/aster_agent_cmd/runtime_turn.rs`（`knowledge_pack.packs[]` 解析）、`src/lib/api/serviceSkills.ts`（Skill bundle summary type guard export） | 可纳入 Knowledge-only 发布，但需要在 commit message 里说明它们服务 `1 persona + N data` 与 seeded Skill projection |
| 明显非 Knowledge 改动 | `scripts/design-canvas-smoke.mjs`、`src/pages/design-canvas-smoke.tsx`、`src-tauri/src/commands/layered_design_cmd.rs`、`src/lib/api/layeredDesignProject.ts`、`src/components/workspace/design/*`、`src/lib/layered-design/*`、`docs/aiprompts/commands.md` 中 layered-design manifest 说明 | 不应进入 Knowledge-only 发布；如果用户选择整体发布，需另写 Design Canvas release scope |
| 计划文件 | `docs/exec-plans/agent-knowledge-implementation-plan.md` | 已更新但当前被 `.gitignore` 忽略；如需随发布进入 Git，必须单独确认调整忽略规则或 `git add -f` |
...
如果选择 **Knowledge-only 发布**，建议先清空当前 index 后按下列 pathspec 重新 staging；这一步仍需人工确认后执行，不应由自动化默认完成：
...
git add -- \
  ...
  "docs/roadmap/knowledge/prd-v2.md" \
  "docs/roadmap/knowledge/prd-v2-diagrams.md" \
  "package.json" \
  "scripts/knowledge-gui-smoke.mjs" \
  "scripts/knowledge-legacy-fallback-report.mjs" \
  "src-tauri/crates/core/src/models/mod.rs" \
  "src-tauri/crates/core/src/models/skill_model.rs" \
  ...
  "src-tauri/src/dev_bridge/dispatcher/knowledge.rs" \
  "src-tauri/src/skills/default_skills.rs" \
  ...
  "src/components/agent/chat/components/FileManager/FileManagerSidebar.tsx" \
  "src/components/agent/chat/components/FileManager/FileManagerSidebar.test.tsx" \
  ...
  "src/lib/dev-bridge/mockPriorityCommands.test.ts" \
...
Knowledge-only 发布不应 staging `scripts/design-canvas-smoke.mjs`、`src/pages/design-canvas-smoke.tsx`、`src-tauri/src/commands/layered_design_cmd.rs`、`src/lib/api/layeredDesignProject.ts`、`src/components/workspace/design/*` 或 `src/lib/layered-design/*`。

当前 release scope report 结果（2026-05-08）：`docs/roadmap/knowledge/evidence/release-scope-report-20260508.json` 输出 `knowledge=75`、`nonKnowledge=30`、`mixed=0`、`unknown=0`。若选择 Knowledge-only 发布，必须确认最终 `git diff --cached --name-only` 不包含 nonKnowledge 列表中的 Design Canvas / layered-design 路径。

只读发布状态检查（2026-05-08）：

- 当前分支：`main`
- 当前 HEAD：`431dc221`（`origin/main`，提交信息 `chore: release v1.30.0`）
- 当前本地 / 远端已有最新 Release：`Lime v1.30.0`，tag `v1.30.0`，GitHub URL `https://github.com/limecloud/lime/releases/tag/v1.30.0`
- `v1.30.0` tag 指向 `28b2a751104a3aa982fdca50f5058aed7db9fee1`，不是当前 dirty worktree 中的 Knowledge v2 改动；因此 Knowledge v2 还没有新的 commit / tag / release 证据。
- 当前版本文件已按完整 worktree 发布准备同步为 `1.31.0`（`package.json`、`package-lock.json`、`packages/lime-cli-npm/package.json`、`src-tauri/Cargo.toml`、`src-tauri/Cargo.lock`、`src-tauri/tauri.conf*.json`），`v1.31.0` tag / GitHub Release 尚未创建；发布前仍需执行 `npm run verify:app-version` 并由用户确认 Git 高风险操作。

下一刀建议：

1. 用真实 Provider 输出文档对照 `谢晶_个人IP知识库v1.0_深澜智能.md` 做人工质量评测；当前已完成一次短资料真实 Provider E2E、短资料输出质量评测、golden sample 质量基线审计（999 行、19 个章节、15 项质量关键词 PASS），并把平台迁移、AI 赋能、新赛道判断等高价值维度回写到个人 IP 模板；“真实访谈风格”fixture 回归已覆盖 document-first、persona wrapper、source anchors 与 brief 退出，但仍不能替代长资料人审。
2. 收紧 `compiled/brief.md` 剩余退出条件：前端、DevBridge mock、smoke、API `compiledView` 与新 pack 写入已改读 / 改写 `compiled/index.json` / splits 或真实 source 文件；旧 pack 回退会产生 migration warning，且重新整理会删除旧 brief 并生成 splits，下一步只剩盘点真实用户 workspace 的历史 fallback 清单。
3. 将 `smoke:knowledge-gui` 继续扩展到可复用的真实 Provider 成功路径脚本；当前已完成一次手工 DevBridge Provider E2E，KnowledgePage 与输入框均已支持可见 `1 persona + N data` 组合选择。
4. 用真实品牌人设、品牌产品、组织 SOP、增长策略和四类运营资料继续做人工质量评测；当前已补八类非个人 pack 的真实风格 fixture 矩阵，覆盖 provenance、document-first splits、Resolver mode、source anchors 与 brief 退出。
5. 等历史 / 未知 pack fallback 清单清零后，删除或停用 `knowledge_builder` deprecated Skill；当前标准 pack 类型已有测试防回流，`knowledge_builder` 只剩未知 / 历史 pack 的最小 document-first JSON 兜底。
6. GitHub commit、tag、push 和 Release 发布属于高风险操作；只有获得明确确认后再执行，不作为本地工程闭环完成的前置条件。
...
9. `src-tauri/resources/default-skills/personal-ip-knowledge-builder/`（persona Builder Skill 事实源）
10. `src-tauri/resources/default-skills/brand-persona-knowledge-builder/`（品牌 persona Builder Skill 事实源）
11. `src-tauri/resources/default-skills/brand-product-knowledge-builder/`（产品 data Builder Skill 事实源）
12. `src-tauri/resources/default-skills/organization-knowhow-knowledge-builder/`（组织经验 data Builder Skill 事实源）
13. `src-tauri/resources/default-skills/growth-strategy-knowledge-builder/`（增长策略 data Builder Skill 事实源）
14. `src-tauri/resources/default-skills/*-operations-knowledge-builder/`（运营类 data Builder Skill 事实源）
