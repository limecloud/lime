...
function buildWorkflowStages(
  ...
  const detailStage: WorkflowStageViewModel = !props.hasSelectedSceneApp
    ...
    : {
        ...
        summary:
          props.activeView === "detail"
            ? props.launchReady
              ? "项目与输入已经具备，可以直接启动，也可以继续校正交付合同与组合步骤。"
              : "继续补齐项目工作区或启动输入，让这条 SceneApp 进入可启动状态。"
            : props.launchReady
              ? "这条 SceneApp 已经具备首轮启动条件，可以直接进入详情页发起结果链。"
              : "这条 SceneApp 还没补齐启动条件，先去详情页完善项目、输入或链接。",
        actionLabel:
  ...
  const governanceStage: WorkflowStageViewModel = !props.hasSelectedSceneApp
    ? {
        ...
        title: "看治理",
        statusLabel: "先选场景",
        summary: "治理页只处理已经选中的 SceneApp，先回目录确定要复盘哪条场景。",
        actionLabel: "先去目录",
        tone: "slate",
        onAction: props.onOpenCatalog,
      }
    : props.hasRuns
      ? {
          ...
          summary:
            props.activeView === "governance"
              ? "最近运行、治理材料和放量判断都集中在这里，可以继续复盘或准备治理动作。"
              : "这条 SceneApp 已经有运行样本，可以直接进入治理页看结果、证据和复核判断。",
          actionLabel:
            props.activeView === "governance" ? "继续复盘" : "进入治理",
      ...
      : {
          ...
          statusLabel: "等待首轮样本",
          summary:
            "这条 SceneApp 还没有首轮运行样本，当前不适合直接复盘，先去详情页跑出第一轮结果链。",
          actionLabel: "先去详情启动",
          tone: "amber",
          onAction: props.onOpenDetail,
        };
