export interface HomeRecommendedSolutionItem {
  id: string;
  title: string;
  summary: string;
  outputHint: string;
  prompt: string;
  badge: string;
  actionLabel: string;
  statusLabel: string;
  statusTone: "emerald";
  recentUsedAt: number | null;
  isRecent: boolean;
  themeTarget?: string;
  shouldEnableWebSearch?: boolean;
  shouldEnableTeamMode?: boolean;
  shouldLaunchBrowserAssist?: boolean;
}

interface HomeRecommendedSolutionUsageRecord {
  solutionId: string;
  usedAt: number;
}

interface HomeRecommendedSolutionDefinition {
  id: string;
  title: string;
  summary: string;
  outputHint: string;
  prompt: string;
  themeTarget?: string;
  shouldEnableWebSearch?: boolean;
  shouldEnableTeamMode?: boolean;
  shouldLaunchBrowserAssist?: boolean;
}

const HOME_RECOMMENDED_SOLUTION_USAGE_STORAGE_KEY =
  "lime:home-recommended-solution-usage:v1";
const MAX_HOME_RECOMMENDED_SOLUTION_USAGE_RECORDS = 12;

const HOME_RECOMMENDED_SOLUTIONS: HomeRecommendedSolutionDefinition[] = [
  {
    id: "web-research-brief",
    title: "网页研究简报",
    summary:
      "快速整理调研范围、关键信息与结论框架，适合先把研究任务落成一版结构化简报。",
    outputHint: "研究提纲 + 结论简报",
    prompt:
      "请围绕这个主题先给我做一版网页研究简报：明确研究目标、关键信息来源、核心发现、风险点，以及接下来最值得继续追踪的问题。",
    shouldEnableWebSearch: true,
  },
  {
    id: "social-post-starter",
    title: "社媒主稿生成",
    summary:
      "先进入社媒专项工作台，再围绕平台语境、结构和表达生成可继续迭代的首稿。",
    outputHint: "社媒首稿 + 平台结构",
    prompt:
      "请先帮我起草一版社媒内容首稿：明确目标受众、平台语境、标题方向、正文结构和可继续扩写的角度。",
    themeTarget: "social-media",
  },
  {
    id: "frontend-concept",
    title: "前端概念方案",
    summary:
      "快速产出信息架构、关键模块与页面关系，适合产品概念、后台台架或工作台原型讨论。",
    outputHint: "IA + 模块方案",
    prompt:
      "请帮我先整理一版前端概念方案：输出信息架构、核心页面、关键模块、交互流程和第一轮组件拆分建议。",
  },
  {
    id: "slide-outline",
    title: "演示提纲草案",
    summary:
      "先拿到一版可讲述的演示结构，覆盖封面、问题、观点、案例与行动建议。",
    outputHint: "PPT 大纲 + 讲述线",
    prompt:
      "请基于这个目标先生成一版演示提纲：包含封面定位、目录、核心论点、案例支撑、结论和下一步行动。",
  },
  {
    id: "browser-assist-task",
    title: "浏览器协助办事",
    summary:
      "适合登录、表单、网页操作和信息采集任务，进入工作区后直接接管浏览器协助链路。",
    outputHint: "浏览器任务执行",
    prompt:
      "请协助我完成一个浏览器任务：先明确目标网页、目标动作、约束条件和预期结果，再进入执行。",
    shouldLaunchBrowserAssist: true,
  },
  {
    id: "team-breakdown",
    title: "多代理拆任务",
    summary:
      "适合需要并行调研、方案拆解或多角色协作的任务，进入后默认启用 team runtime 偏好。",
    outputHint: "任务拆解 + 分工执行",
    prompt:
      "请把这个任务按多代理方式拆解：先定义目标和约束，再拆成并行子任务，明确每个子代理的职责、产出和回收方式。",
    shouldEnableTeamMode: true,
  },
];

function isValidUsageRecord(
  value: unknown,
): value is HomeRecommendedSolutionUsageRecord {
  if (!value || typeof value !== "object") {
    return false;
  }

  const record = value as Partial<HomeRecommendedSolutionUsageRecord>;
  return (
    typeof record.solutionId === "string" &&
    record.solutionId.length > 0 &&
    typeof record.usedAt === "number" &&
    Number.isFinite(record.usedAt)
  );
}

function listHomeRecommendedSolutionUsage(): HomeRecommendedSolutionUsageRecord[] {
  if (typeof window === "undefined") {
    return [];
  }

  try {
    const raw = window.localStorage.getItem(
      HOME_RECOMMENDED_SOLUTION_USAGE_STORAGE_KEY,
    );
    if (!raw) {
      return [];
    }

    const parsed = JSON.parse(raw);
    if (!Array.isArray(parsed)) {
      return [];
    }

    return parsed
      .filter(isValidUsageRecord)
      .sort((left, right) => right.usedAt - left.usedAt)
      .slice(0, MAX_HOME_RECOMMENDED_SOLUTION_USAGE_RECORDS);
  } catch {
    return [];
  }
}

function getHomeRecommendedSolutionUsageMap(): Map<
  string,
  HomeRecommendedSolutionUsageRecord
> {
  return new Map(
    listHomeRecommendedSolutionUsage().map((record) => [record.solutionId, record]),
  );
}

function resolveHomeRecommendedSolutionBadge(
  solutionId: string,
  isRecent: boolean,
): string {
  if (isRecent) {
    return "最近使用";
  }

  if (solutionId === "social-post-starter") {
    return "社媒方案";
  }

  if (solutionId === "browser-assist-task") {
    return "浏览器协助";
  }

  if (solutionId === "team-breakdown") {
    return "多代理";
  }

  return "Claw 方案";
}

export function listHomeRecommendedSolutions(): HomeRecommendedSolutionItem[] {
  const usageMap = getHomeRecommendedSolutionUsageMap();

  return HOME_RECOMMENDED_SOLUTIONS.map((solution, index) => {
    const recentRecord = usageMap.get(solution.id);
    const recentUsedAt = recentRecord?.usedAt ?? null;

    return {
      ...solution,
      badge: resolveHomeRecommendedSolutionBadge(
        solution.id,
        typeof recentUsedAt === "number",
      ),
      actionLabel: "立即开始",
      statusLabel: "可直接开始",
      statusTone: "emerald" as const,
      recentUsedAt,
      isRecent: typeof recentUsedAt === "number",
      _sortIndex: index,
    };
  })
    .sort((left, right) => {
      if (left.recentUsedAt && right.recentUsedAt) {
        if (left.recentUsedAt !== right.recentUsedAt) {
          return right.recentUsedAt - left.recentUsedAt;
        }
      } else if (left.recentUsedAt) {
        return -1;
      } else if (right.recentUsedAt) {
        return 1;
      }

      return left._sortIndex - right._sortIndex;
    })
    .map(({ _sortIndex, ...solution }) => solution);
}

export function recordHomeRecommendedSolutionUsage(solutionId: string): void {
  const nextRecord: HomeRecommendedSolutionUsageRecord = {
    solutionId,
    usedAt: Date.now(),
  };

  const nextRecords = [
    nextRecord,
    ...listHomeRecommendedSolutionUsage().filter(
      (record) => record.solutionId !== solutionId,
    ),
  ].slice(0, MAX_HOME_RECOMMENDED_SOLUTION_USAGE_RECORDS);

  if (typeof window === "undefined") {
    return;
  }

  try {
    window.localStorage.setItem(
      HOME_RECOMMENDED_SOLUTION_USAGE_STORAGE_KEY,
      JSON.stringify(nextRecords),
    );
  } catch {
    // ignore write errors
  }
}
