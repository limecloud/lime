...
import {
  buildCompatQuestionnaireSubmissionPayload,
  buildCompatQuestionnaireA2UI,
  formatCompatQuestionnaireSubmission,
} from "./compatQuestionnaireA2UI";
...
describe("compatQuestionnaireA2UI", () => {
  it("应将结构化问卷正文转换为输入区可渲染的 A2UI 表单", () => {
    const response = buildCompatQuestionnaireA2UI(LEGACY_QUESTIONNAIRE);
  ...
  it("普通回复或内联 a2ui 不应被误识别为 legacy 表单", () => {
    expect(
      buildCompatQuestionnaireA2UI("你可以先给我一个主题，我再继续展开。"),
    ).toBeNull();
    expect(
      buildCompatQuestionnaireA2UI('<a2ui>{"type":"form"}</a2ui>'),
    ).toBeNull();
  });

  it("应兼容 ask/tool_calls 残留问卷正文并转换为可渲染的 A2UI 表单", () => {
    const response = buildCompatQuestionnaireA2UI(
  ...
  it("应将普通中文澄清问题提升为输入区可渲染的 A2UI 表单", () => {
    const response = buildCompatQuestionnaireA2UI(PLAIN_ASK_USER_QUESTIONNAIRE);
  ...
  it("应将 legacy 表单回答格式化为带字段标签的用户摘要", () => {
    ...
    const submission = formatCompatQuestionnaireSubmission(response, {
...
- 这次内容主要面向谁？: 客户`);
  ...
  it("应生成可传给运行时的结构化 elicitation_context 元数据", () => {
    ...
    const payload = buildCompatQuestionnaireSubmissionPayload(response, {
      [componentsByLabel["这次内容主要面向谁？"]]: ["客户"],
    });
