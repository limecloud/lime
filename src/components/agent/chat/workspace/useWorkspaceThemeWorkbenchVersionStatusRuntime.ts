...
interface UseWorkspaceThemeWorkbenchVersionStatusRuntimeParams {
  isThemeWorkbench: boolean;
  themeWorkbenchRunState: "idle" | "auto_running" | "await_user_decision";
  canvasState: CanvasStateUnion | null;
  latestTerminal: ThemeWorkbenchLatestTerminalSummary | null;
  setDocumentVersionStatusMap: Dispatch<
    SetStateAction<Record<string, TopicBranchStatus>>
  >;
}

export function useWorkspaceThemeWorkbenchVersionStatusRuntime({
  isThemeWorkbench,
  themeWorkbenchRunState,
  canvasState,
  latestTerminal,
  setDocumentVersionStatusMap,
}: UseWorkspaceThemeWorkbenchVersionStatusRuntimeParams) {
  useEffect(() => {
