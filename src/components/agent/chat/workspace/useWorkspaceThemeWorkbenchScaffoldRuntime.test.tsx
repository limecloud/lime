...
function renderHook(props?: Partial<HookProps>) {
  ...
  function Probe(currentProps: HookProps) {
    latestValue = useWorkspaceThemeWorkbenchScaffoldRuntime(currentProps);
    return null;
  }
...
describe("useWorkspaceThemeWorkbenchScaffoldRuntime", () => {
  beforeEach(() => {
    (
  ...
  it("应接收并去重 creation_task_submitted 事件", async () => {
    ...
    expect(getValue().themeWorkbenchCreationTaskEvents).toEqual([
    ...
    expect(getValue().themeWorkbenchCreationTaskEvents).toHaveLength(1);
  });
});
