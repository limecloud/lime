...
**功能特性：**
- 检测 Canvas 类型（canvas:document, canvas:video, canvas:design）
...
| canvas:design | design | AI 图层化设计画布 |

旧 `canvas:poster / canvas:music / canvas:novel / canvas:script` 已不再归一到现役 Canvas 类型；需要图层化图片设计时必须使用 `canvas:design` 和 `LayeredDesignDocument`。
