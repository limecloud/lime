---
name: campaign-operations-knowledge-builder
...
2. 读取 `references/campaign-operations-template.md`，按固定章节生成知识库。
3. 缺少关键事实时，先问用户补齐；如果用户要求先生成，则用 `待补充` 标注，不要编造。
4. 提炼事实、流程、角色分工、表达边界、指标口径、风险预案和可复用素材。
5. 用 `references/campaign-operations-quality-checklist.md` 自检，必要时补一节“待补充信息清单”。
...
primaryDocument:
  ...
  content: <按 references/campaign-operations-template.md 生成的完整 Markdown>
...
- 活动策划、预热、执行和复盘
- 跨渠道 Campaign 的素材和节奏协调
- 让 Agent 生成活动方案、提醒清单和复盘报告时提供事实边界
...
- 需要章节骨架时，读取 `references/campaign-operations-template.md`。
- 输出前做质量检查时，读取 `references/campaign-operations-quality-checklist.md`。
