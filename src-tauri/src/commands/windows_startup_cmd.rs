...
pub fn maybe_show_windows_startup_notice(app: &AppHandle) {
    ...
    let message = diagnostics.summary_message.clone().unwrap_or_else(|| {
        "检测到 Windows 启动环境存在阻塞问题，请查看日志并优先重新运行最新的 Windows setup 安装包。".to_string()
    });
...
pub fn collect_windows_startup_diagnostics(app: &AppHandle) -> WindowsStartupDiagnostics {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        match &webview2_version {
            ...
            None => {
                ...
                checks.push(warn_check(
                    ...
                    Some(
                        "如果用户通过便携版或历史安装环境启动失败，请优先重新运行最新的 Windows setup 安装包。".to_string(),
                    ),
                ));
            }
        }
        ...
        let summary_message = if !errors.is_empty() {
            Some(format!(
                "检测到 {} 个阻塞问题：{}。建议先检查目录权限，并优先重新运行最新的 Windows setup 安装包。",
                errors.len(),
                errors.join("；")
            ))
        } else if !warnings.is_empty() {
            Some(format!(
                "检测到 {} 个 Windows 环境提示：{}。如用户反馈启动失败，请优先收集日志并确认使用最新的 Windows setup 安装包。",
                warnings.len(),
                warnings.join("；")
            ))
        } else {
