...
fn build_research_skill_launch_system_prompt(
    ...
    let mut research_request_payload = research_request.clone();
    research_request_payload.insert("runtime_contract".to_string(), runtime_contract.clone());
    let args_payload = serde_json::json!({
        ...
        "research_request": serde_json::Value::Object(research_request_payload.clone()),
    });
    ...
    let request_json = truncate_prompt_text(
        serde_json::to_string(&research_request_payload).unwrap_or_else(|_| "{}".to_string()),
        4_000,
    );
    let has_query = query
        .as_deref()
