...
fn build_deep_search_skill_launch_system_prompt(
    ...
    let mut deep_search_request_payload = deep_search_request.clone();
    deep_search_request_payload.insert("runtime_contract".to_string(), runtime_contract.clone());
    let args_payload = serde_json::json!({
        ...
        "deep_search_request": serde_json::Value::Object(deep_search_request_payload.clone()),
    });
    ...
    let request_json = truncate_prompt_text(
        serde_json::to_string(&deep_search_request_payload).unwrap_or_else(|_| "{}".to_string()),
        4_000,
    );
    let has_query = query
        .as_deref()
