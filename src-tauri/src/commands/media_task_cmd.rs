...
use lime_media_runtime::{
    ...
    write_task_artifact, MediaTaskOutput, MediaTaskType, TaskWriteOptions, DEFAULT_ARTIFACT_ROOT,
};
...
pub(crate) fn retry_media_task_artifact_inner(
    request: MediaTaskLookupRequest,
) -> Result<MediaTaskOutput, String> {
    let project_root_path =
        normalize_required_string(&request.project_root_path, "projectRootPath")?;
    ...
    .map_err(|error| format!("重试媒体任务 artifact 失败: {error}"))
}
...
#[tauri::command]
pub fn retry_media_task_artifact(
    app: AppHandle,
    request: MediaTaskLookupRequest,
) -> Result<MediaTaskOutput, String> {
    let output = retry_media_task_artifact_inner(request)?;
    emit_creation_task_event_if_needed(Some(&app), &output);
    Ok(output)
}
...
pub fn cancel_media_task_artifact(
    ...
    cancel_media_task_artifact_inner(request)
}
...
mod tests {
    ...
    fn media_task_artifact_controls_should_share_same_task_file_protocol() {
        ...
        let retried = retry_media_task_artifact_inner(MediaTaskLookupRequest {
            project_root_path: temp_dir.path().to_string_lossy().to_string(),
            task_ref: created.task_id.clone(),
        })
        .expect("retry task");
        assert_eq!(retried.task_id, created.task_id);
