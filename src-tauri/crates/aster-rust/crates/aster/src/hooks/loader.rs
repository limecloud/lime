...
pub fn load_hooks_from_file(config_path: &Path) -> Result<(), String> {
    ...
    load_hooks_to_registry(config_path, &registry)
}

/// 从项目目录加载 hooks
pub fn load_project_hooks(project_dir: &Path) -> Result<(), String> {
    let registry = global_registry();
    load_project_hooks_to_registry(project_dir, &registry)
}

/// 从项目目录加载 hooks 到指定注册表
pub fn load_project_hooks_to_registry(
    project_dir: &Path,
    registry: &SharedHookRegistry,
) -> Result<(), String> {
    // 检查 .claude/settings.json
    let settings_path = project_dir.join(".claude").join("settings.json");
    if let Err(e) = load_hooks_to_registry(&settings_path, registry) {
        ...
        if let Ok(entries) = fs::read_dir(&hooks_dir) {
            for entry in entries.flatten() {
                ...
                if path.extension().map(|e| e == "json").unwrap_or(false) {
                    if let Err(e) = load_hooks_to_registry(&path, registry) {
