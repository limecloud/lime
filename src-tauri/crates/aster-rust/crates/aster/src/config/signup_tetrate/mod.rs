...
impl PkceAuthFlow {
    ...
    pub async fn exchange_code(&self, code: String) -> Result<String> {
        ...
        let response = client
            ...
            .header("X-Title", "aster")
            .header("Referer", "https://github.com/astercloud/aster-rust")
            .json(&request_body)
            .send()
            .await?;
...
pub fn configure_tetrate(config: &Config, api_key: String) -> Result<()> {
    ...
    config.set_aster_provider("tetrate")?;
    config.set_aster_model(TETRATE_DEFAULT_MODEL)?;
    Ok(())
}
