...
impl TlsConfig {
    ...
    pub fn from_config() -> Result<Option<Self>> {
        ...
        let client_cert_path = config.get_param::<String>("ASTER_CLIENT_CERT_PATH").ok();
        let client_key_path = config.get_param::<String>("ASTER_CLIENT_KEY_PATH").ok();
        ...
        match (client_cert_path, client_key_path) {
            ...
            (Some(_), None) => {
                return Err(anyhow::anyhow!(
                    "Client certificate provided (ASTER_CLIENT_CERT_PATH) but no private key (ASTER_CLIENT_KEY_PATH)"
                ));
            }
            (None, Some(_)) => {
                return Err(anyhow::anyhow!(
                    "Client private key provided (ASTER_CLIENT_KEY_PATH) but no certificate (ASTER_CLIENT_CERT_PATH)"
                ));
            }
            (None, None) => {}
        }

        if let Ok(ca_cert_path) = config.get_param::<String>("ASTER_CA_CERT_PATH") {
