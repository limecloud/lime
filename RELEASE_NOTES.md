...
- 本次发布按当前准备递交的完整暂存内容编写，范围包括版本发布、Skill Forge 主线、只读 HTTP/API 能力草案、Agent Runtime / Evidence Pack、Knowledge v2、AI 图层化设计、GUI 页面与质量脚本。
...
- 新增并落地 `Skill Forge` 研究、路线图、架构图、产品原型与执行计划入口，替换旧 `docs/research/creaoai`、`docs/roadmap/creaoai` 与 `creaoai-*` 执行计划命名。
...
- Skill Forge P5 audit 记录 prompt-to-artifact 样例证据，明确 demo / smoke / evidence 不扩展 P4 runtime，也不打开 scheduler、queue 或 Marketplace。
