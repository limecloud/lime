...
describe("GitHub release asset staging", () => {
  it("同名 macOS updater 资产上传 GitHub Release 前应重命名", () => {
    ...
    writeFile(
      path.join(assetsDir, "aarch64-apple-darwin", "Lime_1.26.0_aarch64.dmg"),
    );
    writeFile(path.join(assetsDir, "x86_64-apple-darwin", "Lime.app.tar.gz"));
    writeFile(
      path.join(assetsDir, "x86_64-apple-darwin", "Lime.app.tar.gz.sig"),
    ...
    writeFile(
      path.join(assetsDir, "x86_64-apple-darwin", "Lime_1.26.0_x64.dmg"),
    );
    ...
    expect(copied.map((item) => item.name).sort()).toEqual(
      [
        "Lime_1.26.0_aarch64.app.tar.gz",
        "Lime_1.26.0_aarch64.app.tar.gz.sig",
        "Lime_1.26.0_aarch64.dmg",
        "Lime_1.26.0_x64.app.tar.gz",
        "Lime_1.26.0_x64.app.tar.gz.sig",
        "Lime_1.26.0_x64.dmg",
        "latest.json",
      ].sort(),
    );
    expect(
      fs.readFileSync(
        path.join(outDir, "Lime_1.26.0_aarch64.app.tar.gz.sig"),
        "utf8",
      ),
    ).toBe("arm-sig");
    expect(
      fs.readFileSync(
        path.join(outDir, "Lime_1.26.0_x64.app.tar.gz.sig"),
        "utf8",
      ),
    ).toBe("x64-sig");
  });
